...
var (
	Database = &cli.StringFlag{
		Name:     "mongo.database",
		Usage:    "The database to write to from MongoDB",
		Value:    "ethlogger",
		Category: flags.MongoCategory,
	}

	Collection = &cli.StringFlag{
		Name:     "mongo.collection",
		Usage:    "The collection to write to in MongoDB",
		Value:    "ethereum",
		Category: flags.MongoCategory,
	}

	MongoURI = &cli.StringFlag{
		Name:     "mongo.uri",
		Usage:    "The URI of the MongoDB server we are interacting with",
...
func MakeChain(ctx *cli.Context, stack *node.Node) (*core.BlockChain, ethdb.Database) {
	...
	chain, err := core.NewBlockChain(chainDb, cache, gspec, nil, engine, vmcfg, nil, nil, mgologger.MongoConfig{})
	if err != nil {
