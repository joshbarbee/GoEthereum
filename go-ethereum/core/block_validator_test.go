...
func TestHeaderVerification(t *testing.T) {
	...
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, ethash.NewFaker(), vm.Config{}, nil, nil, mgologger.MongoConfig{})
	defer chain.Stop()
...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	// Run the header checker for blocks one-by-one, checking for both valid and invalid nonces
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, engine, vm.Config{}, nil, nil, mgologger.MongoConfig{})
	defer chain.Stop()
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, ethash.NewFakeFailer(uint64(len(headers)-1)), vm.Config{}, nil, nil, mgologger.MongoConfig{})
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, ethash.NewFakeDelayer(time.Millisecond), vm.Config{}, nil, nil, mgologger.MongoConfig{})
	defer chain.Stop()
