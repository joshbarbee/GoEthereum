...
func testRepair(t *testing.T, tt *rewindTest, snapshots bool) {
	...
	chain, err := NewBlockChain(db, config, gspec, nil, engine, vm.Config{}, nil, nil, mgologger.MongoConfig{})
	if err != nil {
	...
	newChain, err := NewBlockChain(db, nil, gspec, nil, engine, vm.Config{}, nil, nil, mgologger.MongoConfig{})
	if err != nil {
...
func TestIssue23496(t *testing.T) {
	...
	chain, err = NewBlockChain(db, nil, gspec, nil, engine, vm.Config{}, nil, nil, mgologger.MongoConfig{})
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	defer chain.Stop()
