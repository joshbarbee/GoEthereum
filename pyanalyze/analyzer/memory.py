...
class Memory:
    ...
    def __init__(self, path) -> None:
        ...
        self.ops: Dict[
            str, pd.DataFrame
        ] = {}  # dict of mapping between opcode and pandas dataframe of opcode
    ...
    def __load_facts(self) -> None:
        ...
        """
        self.ops_df = pd.read_csv(
            self.path + "/opAll.facts",
            sep="\t",
            header=None,
            names=[
                "loc",
        ...
        _origin = pd.read_csv(
            self.path + "/sc_addr.facts", sep="\t", header=None, names=["origin_addr"]
        )
    ...
    def reduce_instr(
        self, df: pd.DataFrame, instr: str, cn_shift: int = 0, invert: bool = False
    ) -> pd.DataFrame:
        """
        ...
        *invert is a boolean. False is default. Enabled true will make it such that instructions
        ...
        df.loc[:, "contains_instr"] = df.apply(
            lambda row: parse_row(row["def"], row["cn"]), axis=1
        )
    ...
    def find_instr_depth(
        ...
        bounded_instrs = all_instrs[
            all_instrs["depth"].between(min_depth, max_depth, inclusive="both")
        ]
        ...
        return (
            bounded_instrs.copy()
        )  # we create a copy so that any reductions can be safely made on the data
    ...
    def reduce_value(
        ...
        if df2 is not None:
            df1.loc[:, "use_val"] = df1.apply(
                lambda row: locate_variable(row[df1_comparator]), axis=1
            )

            df2.loc[:, "use_val"] = df2.apply(
                lambda row: locate_variable(row[df2_comparator]), axis=1
            )

            df1.loc[:, "val_matches"] = df1.apply(
                lambda row: df2.loc[
                    (df2["use_val"] == row["use_val"]) & (df2["loc"] > row["loc"])
                ]["loc"].to_list(),
                axis=1,
            )
    ...
    def reduce_addr(
        self, df1: pd.DataFrame, df2: pd.DataFrame, inplace: bool = True
    ) -> pd.DataFrame:
        """
        ...
        df1.loc[:, "addr"] = df1.apply(
            lambda row: locate_addr(row["cn"], row["depth"]), axis=1
        )

        df2.loc[:, "addr"] = df2.apply(
            lambda row: locate_addr(row["cn"], row["depth"]), axis=1
        )

        df1.loc[:, "addr_matches"] = df1.apply(
            lambda row: df2.loc[
                (df2["addr"] == row["addr"]) & (df2["loc"] > row["loc"])
            ]["loc"].to_list(),
            axis=1,
        )
    ...
    def reduce_origin(
        ...
        df1.loc[:, "origin"] = df1.apply(
            lambda row: locate_origin_op(row["def"]), axis=1
        )
    ...
    def reduce_use(
        self, df1: pd.DataFrame, df1_comp: str, df2: pd.DataFrame, df2_comp: str
    ):
        """
