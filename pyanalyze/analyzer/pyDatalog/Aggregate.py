...
class Aggregate(object):
    ...
    def __init__(
        self, Y=None, group_by=tuple(), for_each=tuple(), order_by=tuple(), sep=None
    ):
        # convert for_each=Z to for_each=(Z,)
        self.Y = Y
        self.group_by = (
            (group_by,) if isinstance(group_by, Expression) else tuple(group_by)
        )
        self.for_each = (
            (for_each,) if isinstance(for_each, Expression) else tuple(for_each)
        )
        self.order_by = (
            (order_by,) if isinstance(order_by, Expression) else tuple(order_by)
        )

        # recover the negated variable in order_by
        self.order_by = tuple(
            [e.__dict__.get("_pyD_variable", e) for e in self.order_by]
        )

        if not all(
            [
                isinstance(e, Term) and e.is_variable()
                for e in self.group_by + self.for_each + self.order_by
            ]
        ):
            raise util.DatalogError(
                "Arguments of aggregate must be variable(s).", None, None
            )

        if sep and not isinstance(sep, util.string_types):
            raise util.DatalogError(
                "Separator in aggregation must be a string", None, None
            )
        self.sep = sep
        ...
        for kw in self.required_kw:
            ...
            if arg is None or (isinstance(arg, tuple) and arg == tuple()):
                raise util.DatalogError(
                    "Error: argument missing in aggregate", None, None
                )

        # used to create literal.
        self.args = (
            ((Y,) if Y is not None else tuple())
            + self.group_by
            + self.for_each
            + self.order_by
    ...
    def make_literal_for(self, function, operator):
        if operator != "==":
            raise util.DatalogError(
                "Aggregate operator can only be used with equality.", None, None
            )
        ...
        terms = (
            [Term.make_for_prefix(function._pyD_name)]
            + list(function._pyD_keys)
            + [result]
        )  # prefixed
        ...
        self.slice_for_each = [
            variables[variable._pyD_name] for variable in self.for_each
        ]
        self.reversed_order_by = [
            variables[variable._pyD_name] for variable in self.order_by
        ][::-1]
        self.reverse_order = [variable._pyD_negated for variable in new_terms[:-1]]
        if isinstance(
            self, Rank
        ):  # can't use required_kw because rank does not require group_by
            self.slice_group_by = [
                variables[variable._pyD_name] for variable in self.group_by
            ]
        else:
            self.slice_group_by = [
                variables[Expression._pyD_for(variable)._pyD_name]
                for variable in function._pyD_keys
                if isinstance(variable, Term)
            ]
        self.slice_to_variabilize = [
            variables[variable._pyD_name]
            for variable in function._pyD_keys
            if isinstance(variable, Term)
    ...
    def fact_candidate(self, subgoal, row):
        if row is not None:
            ...
            row[self.index_first_arg : -1] = [""] * (
                len(row) - self.index_first_arg - 1
            )
    ...
    def sort_result(self, result):
        ...
        for i in self.reversed_order_by:
            result.sort(
                key=lambda literal, i=i, self=self: literal[i].id,
                reverse=self.reverse_order[i],
            )
        # then sort per group_by
        result.sort(
            key=lambda literal, self=self: [literal[i].id for i in self.slice_group_by]
        )
...
class Linear_regression(Aggregate):
    ...
    def value(self):
        ...
        a = (covariance - (sum_x * sum_y) / length) / (
