...
def test():
    ...
    @pyDatalog.program()
    def _lambda():
        assert ask((X == "world") & (Y == (lambda X: "hello " + X))) == set(
            [("world", "hello world")]
        )
        split(X, Y, Z) <= (X == Y + "-" + Z)
        assert ask(split(X, "a", "b")) == set([("a-b",)])
        split(X, Y, Z) <= (Y == (lambda X: X.split("-")[0])) & (
            Z == (lambda X: X.split("-")[1])
        )
        assert ask(split("a-b", Y, Z)) == set(
            [
                (
                    "a",
                    "b",
    ...
    def function():
        ...
        assert (
            ask(plus[plus[1, 2] + 1, 2 + plus[2, 3]] == plus[4, plus[2, 5]])
        ) == set([()])
    ...
    def sum_():
        ...
        assert ask((a_sum[X] == 2) & (p(X, Z, Y))) == set(
            [("a", "c", 1), ("a", "b", 1)]
        )
    ...
    def max_():
        ...
        # issue#4
        (prev_zero[X, Y] == max_(Z, order_by=Z)) <= Z.in_(range_(len(X))) & (Z < Y) & (
            X[Z] == 0
        )
        (next_one[X, Y] == min_(Z, order_by=Z)) <= Z.in_(range_(len(X))) & (Z > Y) & (
            X[Z] == 1
        )
    ...
    def rank():
        ...
        assert ask(place[Person] == Rank) == set(
            [("Superman", 2), ("Tom", 0), ("Jerry", 1)]
        )
        ...
        assert ask(place[Person] == Rank) == set(
            [("Superman", 2), ("Tom", 0), ("Jerry", 1), ("Phil", 0)]
        )
        assert ask(place["Phil"] == Rank) == set([(0,)])

        # reverse ranking
        (reverse_place[Person] == rank(order_by=-Score)) <= score(Person, Score)
        assert ask(reverse_place[Person] == Rank) == set(
            [("Superman", 0), ("Tom", 2), ("Jerry", 1)]
        )
        assert ask(reverse_place["Superman"] == Rank) == set([(0,)])
        assert ask(reverse_place[Person] == 2) == set([("Tom",)])

        # multiple keys, group_by
        (multi_place[Person, Score] == rank(group_by=Person, order_by=Score)) <= score(
            Person, Score
        )
        assert ask(multi_place[Person, Score] == Rank) == set(
            [("Superman", 10, 0), ("Tom", 3, 0), ("Jerry", 8, 0)]
        )
        assert ask(multi_place["Superman", Score] == Rank) == set(
            [
                (
                    10,
                    0,
    ...
    def running_sum():
        ...
        (
            balance[Account, Y, M, D]
            == running_sum(Amount, group_by=Account, order_by=(Y, M, D))
        ) <= movement(Account, Y, M, D, Amount)
        assert ask(balance[Account, Y, M, D] == Amount) == set(
            [
                ("Account1", 2014, 1, 1, 10),
                ("Account1", 2014, 1, 2, 2),
                ("Account1", 2014, 1, 3, 0),
                ("Account2", 2014, 1, 1, 10),
                ("Account2", 2014, 1, 2, 5),
            ]
        )
        assert ask(balance["Account1", Y, M, D] == Amount) == set(
            [(2014, 1, 1, 10), (2014, 1, 2, 2), (2014, 1, 3, 0)]
        )
        assert ask(balance[Account, 2014, 1, 2] == Amount) == set(
            [("Account1", 2), ("Account2", 5)]
        )
        ...
        assert ask(balance[Account, Y, M] == Amount) == set(
            [("Account1", 2014, 1, 0), ("Account2", 2014, 1, 5)]
        )
        assert ask(balance["Account1", Y, M] == Amount) == set([(2014, 1, 0)])
        assert ask(balance[Account, 2014, 1] == Amount) == set(
            [("Account1", 0), ("Account2", 5)]
        )
    ...
    def combined():
        ...
        (t[None] == tuple_(Row, order_by=Y)) <= (
            (share[X] == S) & (rank[X] == Y) & (Row == (X, S, Y))
        )

        assert ask(t[None] == Text) == set(
            [((("a", 0.3, 0), ("b", 0.2, 1), ("c", 0.1, 2)),)]
        )
    ...
    try:
        ...
        if (
            e_message
            != "Predicate without definition (or error in resolver): A.z[1]==/3"
        ):
            print(e_message)
    else:
        assert False
    ...
    assert_error(
        "ask(z(a))", "Predicate without definition \(or error in resolver\): z/1"
    )
    assert_error(
        "+ farmer(farmer(moshe))",
    ...
    assert_error(
        "+ manager[Mary]==John",
        "Did you mean to assert or retract a fact \? Please add parenthesis.",
    )
    assert_error(
        "ask(- manager[X]==1)",
        "Did you mean to assert or retract a fact \? Please add parenthesis.",
    ...
    assert_error(
        "p(X) <= (manager[X]== min(X))", "Error: argument missing in aggregate"
    )
    assert_error(
        "p(X) <= (manager[X]== max(X, order_by=X))",
        "Aggregation cannot appear in the body of a clause",
    )
    assert_error(
        "q(min(X, order_by=X)) <= p(X)", "Syntax error: Incorrect use of aggregation\."
    )
    assert_error(
    ...
    assert_error(
        ...
        "Arguments of aggregate must be variable\(s\).",
    )
    ...
    assert_error(
        "ask((Z.b[X]=='wa') & (X.b[1]<Y))",
        "Error: right hand side of comparison must be bound: ",
    )
    assert_ask(
        "A.u[X]==Y",
        "Predicate without definition \(or error in resolver\): A.u\[1\]==/3",
    )
    assert_ask(
        "A.u[X,Y]==Z",
        "Predicate without definition \(or error in resolver\): A.u\[2\]==/4",
    )
    ...
    class Employee(Base):  # --> Employee inherits from the Base class
        ...
        def Employee(self):
            (Employee.manager[X] == Y) <= (Employee.manager_name[X] == Z) & (
                Z == Employee.name[Y]
            )
            ...
            Employee.indirect_manager(X, Y) <= (
                Employee.manager[X] == Z
            ) & Employee.indirect_manager(Z, Y) & (Y != None)
