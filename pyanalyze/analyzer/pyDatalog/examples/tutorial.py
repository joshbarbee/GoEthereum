...
pyDatalog.create_terms(
    "parent,bill,ancestor,descendents,manager, X,Y,Z,N,N1,F,  factorial, first_remainder, odd,even, _split"
)
...
_split(X, Y, Z) <= (X == Y + "-" + Z)
_split(X, Y, Z) <= (Y == (lambda X: X.split("-")[0])) & (
...
class Employee(
    pyDatalog.Mixin
):  # --> Employee inherits the pyDatalog capability to use logic clauses
    def __init__(
        self, name, manager, salary
    ):  # method to initialize Employee instances
        super(
            Employee, self
        ).__init__()  # calls the initialization method of the Mixin class
        self.name = name
...
print(
    (Employee.salary[X] == 6300) >= X
)  # prints the first value of X in the query, in one line
