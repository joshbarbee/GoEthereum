...
def assert_fact(predicate_name, *args):
    ...
    +pyParser.Literal.make(
        predicate_name, [pyParser.Expression._pyD_for(arg) for arg in args]
    )
    ...
    def __call__(self, *arguments, **keyargs):
        for a in arguments:
            if isinstance(a, pyParser.Expression):
                assert (
                    not keyargs
                ), "Sorry, key word arguments are not supported yet"  # TODO
...
def create_terms(*args):
    ...
    try:
        ...
        args = [
            ...
            for arg in (
                arglist.split(",")
                if isinstance(arglist, util.string_types)
                else [arglist]
            )
        ]
            ...
            if words[0] in b:  # if it 's a builtin
                ...
                if len(words) == 2:  # e.g. kkecxivarenx.len
                    raise util.DatalogError(
                        "Unknown variable : %s" % words[0], None, None
                    )
...
class metaMixin(type):
    ...
    def __init__(cls, name, bases, dct):
        ...
        cls.has_SQLAlchemy = any(
            base.__module__
            in ("sqlalchemy.ext.declarative", "sqlalchemy.ext.declarative.api")
            for base in bases
        )
        ...
        def _getattr(self, attribute):
            ...
            if not attribute == "__iter__" and not attribute.startswith("_sa_"):
                ...
                terms = (
                    pyParser.Term("_pyD_class", forced_type="constant"),
                    self,
                    pyParser.Term("X"),
                )  # prefixed
                literal = pyParser.Literal.make(
                    predicate_name, terms
                )  # TODO predicate_name[:-2]
        ...
        def __lt__(
            self, other
        ):  # needed for sorting in aggregate functions using Python 3
        ...
        if len(terms) == 3:  # prefixed
            ...
            if X.is_const():
                ...
                if (
                    not Y.is_const()
                    or not operator
                    or pyEngine.compare(Y1, operator, Y.id)
                ):
                    yield (
                        terms[0],
                        X.id,
                        Y.id if Y.is_const() else Y1 if operator == "==" else None,
                    )
                ...
                if cls.session:
                    ...
                    if Y.is_const():
                        q = q.filter(
                            pyEngine.compare(getattr(cls, attr_name), operator, Y.id)
                        )
                    for r in q:
