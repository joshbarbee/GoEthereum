...
class Operation(Term):
    ...
    def __init__(self, lhs, operator, rhs):
        ...
        self.operator_id = (
            "l" + str(Operation.counter.next())
            if isinstance(self.operator, type(util.LAMBDA))
    ...
    def subst(self, env):  # unify
        ...
        try:
            if (
                self.operator == "["
                and isinstance(lhs, (VarTuple, Const))
                and rhs.is_const()
            ):
    ...
    def __new__(cls, pred_name, arity):
        ...
        with Pred.lock:
            ...
            if o is Interned.notFound:
                ...
                words = o.name.split(".")
                o.prefix, o.suffix = (
                    (words[0], words[1].split("[")[0]) if 1 < len(words) else ("", "")
                )
    ...
    def is_known(cls, pred_id):
        ...
        if prefix in Class_dict:
            for cls in Class_dict[prefix].__mro__:
                rebased = pred_id.replace(
                    prefix, cls.__name__, 1
    ...
    def parent_classes(self):
        ...
        if self._class():
            for cls in self._cls.__mro__:
                if cls.__name__ in Class_dict and cls.__name__ not in (
                    "Mixin",
                    "object",
                ):
                    yield cls
...
class Literal(object):
    ...
    def rebased(self, parent_class):
        ...
        return self._renamed(
            re.sub(
                "^((~)?)" + pred._class().__name__ + ".",
                r"\1" + parent_class.__name__ + ".",
                pred.name,
            )
        )
    ...
    def __str__(self):
        return "%s(%s)" % (
            self.pred.name,
            ",".join([str(term).replace("'_pyD_class'", "*") for term in self.terms]),
        )
    ...
    def unify(self, other):  # unify
        ...
        for term, otherterm in zip(self.terms, other.terms):
            ...
            other_i = (
                otherterm if isinstance(otherterm, Const) else otherterm.chase(env)
            )
    ...
    def ask(self):
        ...
        Ts.Tasks, Ts.Recursive_Tasks, Ts.Subgoals, Ts.Goal = (
            list(),
            deque(),
            {},
            Subgoal(self),
        ...
        if Ts.Goal.facts is True:
            return True
        result = [
            tuple(term.id for term in literal.terms)
...
class Clause(object):
    ...
    def __str__(self):
        return "%s <= %s" % (
            str(self.head),
            "&".join(str(literal) for literal in self.body),
        )

    def __repr__(self):
        return (
            "%s <= %s"
            % (str(self.head), "&".join(str(literal) for literal in self.body))
        )[:50]
    ...
    def get_id(self):  # id
        ...
        if self.id is None:  # cached
            if not self.body:  # if it is a fact
                ...
                self.id = (self.head.get_id(),) + tuple(
                    bodi.get_id() for bodi in self.body
                )
        return self.id
    ...
    def subst(self, env, parent_class=None):
        ...
        if not env and not parent_class:
            return self
        if not parent_class:
            return Clause(self.head.subst(env), [bodi.subst(env) for bodi in self.body])
        return Clause(
            self.head.subst(env).rebased(parent_class),
    ...
    def rename(self):
        ...
        env = {}
        return Clause(self.head.shuffle(env), [bodi.shuffle(env) for bodi in self.body])
...
def relevant_clauses(literal):
    ...
    if (
        result is None
    ):  # no constants found in literal, thus could not filter literal.pred.deb
...
class Subgoal(object):
    ...
    def search(self):
        ...
        for _class in literal0.pred.parent_classes():
            ...
            if _class:
                ...
                method_name = "_pyD_%s%i" % (
                    literal.pred.suffix,
                    int(literal.pred.arity - 1),
                )  # prefixed
            ...
            if literal.pred.prim:  # X==Y, X < Y+Z
                ...
                literal.pred.comparison
            ):  # p[X]<=Y => consider translating to (p[X]==Y1) & (Y1<Y)
                literal1 = literal.equalized()
                if (
                    literal1.pred.id in Logic.tl.logic.Db
                ):  # equality has a datalog definition
                    Y1 = Fresh_var()
                    literal1.terms[-1] = Y1
        ...
        if class0:  # a.p[X]==Y, a.p[X]<y, to access instance attributes
            ...
            literal.pred.comparison
            and len(terms) == 3
            and terms[0].is_const()
            and terms[0].id != "_pyD_class"
            and terms[1].is_const()
        ):  # X.a[1]==Y
            ...
            if terms[2].is_const() and compare(v, literal.pred.comparison, terms[2].id):
                ...
                raise util.DatalogError(
                    "Error: right hand side of comparison must be bound: %s"
                    % literal.pred.id,
                    None,
                    None,
                )
    ...
    def add_clause(self, clause):
        ...
        if self.is_done:  # for speed
            ...
            return (
                self.next_step()
            )  # no need to keep looking if THE answer is found already
        if not clause.body:
    ...
    def fact(self, literal):
        ...
        """
        all_const = (
            True  # Cython equivalent for all(t.is_const() for t in literal.terms)
        )
        if literal is not True:
            for t in literal.terms:
                if not (
                    isinstance(t, Const) or t.is_const()
                ):  # use isinstance for speed
                    all_const = False
                    break
    ...
    def rule(self, clause, selected):
        ...
        if sg != None:  # selected subgoal exists already
            if sg.facts is True:
                ...
                for (
                    fact
                ) in sg.facts.values():  # catch-up with facts already established
    ...
    def searching(self, subgoal):
        ...
        if all(subgoal != waiter[0] for waiter in self.waiters):
            ...
            if (
                subgoal.tasks
            ):  # place first subgoal task on top of stack, for immediate use
...
def compare_primitive(literal, subgoal):
    ...
    if not x.is_const():
        if literal.pred.name == "_pyD_in":
            ...
            raise util.DatalogError(
                "Error: left hand side of comparison must be bound: %s"
                % literal.pred.id,
                None,
                None,
            )
        ...
        raise util.DatalogError(
            "Error: right hand side of comparison must be bound: %s" % literal.pred.id,
            None,
            None,
        )
