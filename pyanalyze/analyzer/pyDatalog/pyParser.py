...
class LazyListOfList(LazyList):
    ...
    def __eq__(self, other):
        """uses set comparison"""
        return other is True if self.data is True else set(self.data) == set(other)
    ...
    def __unicode__(self):
        ...
        widths = [
            max(len(util.unicode_type(x)) for x in column)
            for column in zip(*(self._data))
        ]
        widths = [
            max(widths[i], len(util.unicode_type(self.variables[i])))
...
class Expression(object):
    ...
    def _pyD_for(cls, operand):
        ...
        if isinstance(operand, (Expression, Aggregate)):
            return operand
        if (
            isinstance(operand, type(lambda: None))
            and (operand.__name__ if PY3 else operand.func_name) == "<lambda>"
        ):
            return Operation(
                None,
                operand,
                [Term(var) for var in getattr(operand, func_code).co_varnames],
            )
    ...
    def __eq__(self, other):
        if (
            isinstance(self, Operation)
            and self._pyD_operator in "+-"
            and self._pyD_lhs._pyD_value == 0
        ):
...
class Term(threading.local, Expression, LazyList):
    ...
    def __init__(self, name="??", forced_type=None):
        ...
        if isinstance(name, (list, tuple, util.xrange)):
            ...
            forced_type == "constant"
            or isinstance(name, (int, float, bool))
            or name is None
            or (
                (
                    isinstance(name, util.string_types)
                    and name[0] not in string.ascii_uppercase + "_"
                    and not "." in name
                )
            )
        ):
            ...
            if 0 < index:
                self._pyD_lua = pyEngine.Operation(
                    pyEngine.Var(name[:index]), ".", pyEngine.Const(name[index:])
                )
            else:
        ...
        # manage the aggregate functions
            ...
            if isinstance(args[0], Term):
                return Aggregate.Sum(
                    args[0], for_each=kwargs.get("for_each", kwargs.get("key", []))
                )
            else:
                return sum(args)
            ...
            return Aggregate.Linear_regression(
...
class Function(Expression):
    ...
    def __unicode__(self):
        return "%s[%s]" % (
            self._pyD_name.split("[")[0],
            ",".join(util.unicode_type(key) for key in self._pyD_keys),
        )
...
class Operation(Expression):
    ...
    def __init__(self, lhs, operator, rhs):
        ...
        self._pyD_precalculations = pre_calculations(
            (self._pyD_lhs, self._pyD_rhs)
        )  # TODO test for slice, len
    ...
    def __unicode__(self):
        return (
            "("
            + util.unicode_type(self._pyD_lhs._pyD_name)
            + self._pyD_operator
            + util.unicode_type(self._pyD_rhs._pyD_name)
            + ")"
        )
...
class Literal(object):
    ...
    def __init__(self, predicate_name, args, kwargs, prearity=None, aggregate=None):
        ...
        cls_name = (
            self.predicate_name.split(".")[0].replace("~", "")
            if 1 < len(self.predicate_name.split("."))
            else ""
        )
        if (
            pyEngine.Class_dict.get(cls_name)
            and 2 <= len(self.args)
            and not isinstance(self.args[1], Term)
            and cls_name not in [c.__name__ for c in self.args[1].__class__.__mro__]
        ):
            raise TypeError(
                "Object is incompatible with the class that is queried."
            )  # prefixed
        ...
        for arg in self.args:
            if isinstance(arg, Literal):
                raise util.DatalogError(
                    "Syntax error: Literals cannot have a literal as argument : %s%s"
                    % (self.predicate_name, self.terms),
                    None,
                    None,
                )
    ...
    def make(cls, predicate_name, terms, kwargs=None, prearity=None, aggregate=None):
        ...
        if "!" in predicate_name:  # pred e.g. aggregation literal
            return precalculations & HeadLiteral(
                predicate_name, terms, kwargs, prearity, aggregate
            )
        else:
            return precalculations & Query(
                predicate_name, terms, kwargs, prearity, aggregate
            )
    ...
    def make_for_comparison(cls, self, operator, other):
        ...
        if isinstance(self, Function):
            ...
            terms = (
                [Term.make_for_prefix(self._pyD_name)] + list(self._pyD_keys) + [other]
            )  # prefixed
...
class Query(Literal, LazyListOfList):
    ...
    def __init__(
        self, predicate_name, terms, kwargs=None, prearity=None, aggregate=None
    ):
...
class Body(LazyListOfList):
    ...
    def literal(self):
        ...
        if (
            len(self.literals) == 1
        ):  # determine the literal prearity in case of a single literal
        ...
        literal = Literal.make(
            "_pyD_query" + util.unicode_type(Body.counter.next()),
    ...
    def ask(self):
        ...
        transposed = (
            list(zip(*(self._data))) if isinstance(self._data, list) else None
        )  # transpose result
    ...
    def visit_Compare(self, node):
        ...
        if sys.version_info < (3, 5):
            newNode = ast.Call(
                ast.Attribute(
                    var,
                    "in_" if isinstance(node.ops[0], ast.In) else "not_in_",
                    var.ctx,
                ),  # func
                [comparators],  # args
                [],  # keywords
...
def load(code, newglobals=None, defined=None, function="load"):
    ...
    defined.add("None")
    for name in set(code.co_names).difference(
        defined
    ):  # for names that are not defined
...
def add_program(func):
    ...
    defined = set(code.co_varnames).union(
        set(newglobals.keys())
...
class Answer(object):
    ...
    def __eq__(self, other):
        return (
            other == True
            if self.answers is True
            else other == set(self.answers)
            if self.answers
            else other is None
        )
    ...
    def __str__(self):
        return (
            "True"
            if self.answers is True
            else util.cast_to_str(util.unicode_type(set(self.answers)))
